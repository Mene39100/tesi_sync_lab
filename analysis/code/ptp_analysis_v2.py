...
- For the same metric (e.g., boundary path_delay_ns), low/medium/high must share
  the same y-axis limits to enable visual comparisons across scenarios.
"""
...
def _compute_ylim_symmetric(values: List[pd.Series], pad: float = 0.05) -> Optional[YLim]:
    """
    For offset-like metrics that can be negative/positive.
    Uses symmetric limits around 0 based on global max absolute value.
    """
...
def _compute_ylim_positive(values: List[pd.Series], pad: float = 0.05) -> Optional[YLim]:
    """
    For delay/jitter/rms-like metrics that are >= 0.
    Forces y-axis to start at 0 and end at global max.
    """
...
def build_common_ylims_ptp(runs: List[ParsedRun]) -> YLimMap:
    """
    Builds per-(role,metric) y-limits shared across scenarios.
    """
    ylims: YLimMap = {}

    # Boundary runs
    b_runs = [r for r in runs if r.role == "boundary" and not r.samples.empty]
    if b_runs:
        off = _compute_ylim_symmetric([r.samples["offset_ns"] for r in b_runs])
        if off is not None:
            ylims[("boundary", "offset_ns")] = off

        dly = _compute_ylim_positive([r.samples["path_delay_ns"] for r in b_runs])
        if dly is not None:
            ylims[("boundary", "path_delay_ns")] = dly

        # Post-lock offset (s2 only)
        post_series: List[pd.Series] = []
        for r in b_runs:
            if "servo_state" in r.samples.columns:
                post = r.samples[r.samples["servo_state"] == 2]
                if not post.empty:
                    post_series.append(post["offset_ns"])
        post_off = _compute_ylim_symmetric(post_series) if post_series else None
        if post_off is not None:
            ylims[("boundary", "offset_ns_postlock_s2")] = post_off

    # Client runs
    c_runs = [r for r in runs if r.role == "client" and not r.samples.empty]
    if c_runs:
        rms = _compute_ylim_positive([r.samples["rms_ns"] for r in c_runs if "rms_ns" in r.samples])
        if rms is not None:
            ylims[("client", "rms_ns")] = rms

        delay_series: List[pd.Series] = []
        for r in c_runs:
            if "path_delay_ns" in r.samples.columns:
                delay_series.append(r.samples["path_delay_ns"])
        c_dly = _compute_ylim_positive(delay_series) if delay_series else None
        if c_dly is not None:
            ylims[("client", "path_delay_ns")] = c_dly
...
def _plot_series(
    ...
    If ylim is provided, it forces the y-axis scale to enable visual comparisons
    across scenarios for the same metric.
    """
...
def plot_boundary(run: ParsedRun, outdir: Path, ylims: YLimMap) -> None:
    """
    Boundary plots:
    - offset vs time
    - path delay vs time
    ...
    _plot_series(
        ...
        ylim=ylims.get(("boundary", "offset_ns")),
    )
    ...
    _plot_series(
        ...
        ylim=ylims.get(("boundary", "path_delay_ns")),
    )
    ...
    if "servo_state" in s.columns:
        ...
        if not post.empty:
            _plot_series(
                ...
                ylim=ylims.get(("boundary", "offset_ns_postlock_s2")),
            )


def plot_client(run: ParsedRun, outdir: Path, ylims: YLimMap) -> None:
    """
    Client plots:
    - RMS vs time
    ...
    if "path_delay_ns" in s.columns and s["path_delay_ns"].notna().any():
        _plot_series(
            ...
            ylim=ylims.get(("client", "path_delay_ns")),
        )
...
def main() -> None:
    ...
    # 2) Compute shared y-limits for each metric across scenarios.
    ylims = build_common_ylims_ptp(runs)

    # 3) Produce plots using shared y-axis scales.
    for run in runs:
        if run.role == "boundary":
            plot_boundary(run, plots_dir, ylims)
        else:
            plot_client(run, plots_dir, ylims)

    # 4) Aggregate summaries across runs.
